    ...
    struct ErrorLocalization {
        ...
        func testErrorDescriptions() {
            let testCases: [(ValidationError, String)] = [
                ...
                (ValidationError(field: "bio", rule: .tooLong(max: 100)),
                 "bio is too long (maximum 100 characters)"),
                (ValidationError(field: "password", rule: .tooShort(min: 8)),
                 "password is too short (minimum 8 characters)"),
                (ValidationError(field: "age", rule: .outOfRange(min: 18, max: 65)), "age must be between 18 and 65"),
                (ValidationError(field: "date", rule: .invalidFormat(reason: "must be in ISO format")),
                 "date has invalid format: must be in ISO format"),
                (ValidationError(field: "status", rule: .businessRule(reason: "Cannot change from completed")),
                 "Cannot change from completed"),
                (ValidationError(field: "field", rule: .custom(message: "Custom error message")),
                ...
                (ValidationError(field: "confirmEmail", rule: .notMatching(otherField: "email")),
        ...
        func testRecoverySuggestions() {
            let testCases: [(ValidationError, String?)] = [
                ...
                (ValidationError(field: "password", rule: .tooShort(min: 8)),
                 "Lengthen password to at least 8 characters"),
                (ValidationError(field: "age", rule: .outOfRange(min: 18, max: 65)),
                 "Choose a value between 18 and 65"),
                (ValidationError(field: "date", rule: .invalidFormat(reason: "must be ISO")),
                 "Check the format: must be ISO"),
                ...
                (ValidationError(field: "email", rule: .invalidEmail),
                ...
                (ValidationError(field: "confirmEmail", rule: .notMatching(otherField: "email")),
                 "Ensure confirmEmail matches email")
            ]
